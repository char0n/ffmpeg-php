...
namespace FFmpegPHPTest {
    ...
    class MovieTest extends \PHPUnit_Framework_TestCase {
        ...
        public static function setUpBeforeClass() {
            self::$moviePath   = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'test.mp4';
            self::$audioPath   = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'test.wav';
            self::$noMediaPath = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'test1.txt';
        }
        ...
        public function testPersistentResourceSimulation() {
            \PHP_Timer::start();
            $movie   = new Movie(self::$moviePath, new FFmpegOutputProvider('ffmpeg', true));
            $movie   = new Movie(self::$moviePath, new FFmpegOutputProvider('ffmpeg', true));
            $movie   = new Movie(self::$moviePath, new FFmpegOutputProvider('ffmpeg', true));
        ...
        public function testGetFrameNumber() {
            ...
            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getNextKeyFrame());
            $this->assertInternalType('int', $this->movie->getFrameNumber(), 'Frame number is of integer type');
            $this->assertEquals(1, $this->movie->getFrameNumber(), 'Frame number should be int(1)');
            ...
            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getFrame());
            $this->assertInternalType('int', $this->movie->getFrameNumber(), 'Frame number is of integer type');
        ...
        public function testGetFrame() {
            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getFrame(), 'Frame is of FFmpegFrame type');
            $this->assertEquals(1, $this->movie->getFrameNumber(), 'Frame number should be int(1)');

            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getFrame(25), 'Frame is of FFmpegFrame type');
        ...
        public function testGetNextKeyFrame() {
            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getNextKeyFrame(), 'KeyFrame is of FFmpegFrame type');
            $this->assertEquals(1, $this->movie->getFrameNumber(), 'Frame number should be int(1)');
            $this->assertInstanceOf('\FFmpegPHP\Frame', $this->movie->getNextKeyFrame(), 'Next key frame is of FFmpegFrame type');
            $this->assertEquals(2, $this->movie->getFrameNumber(), 'Frame number should be int(2)');
        }
